     ...
     responseSender:(RCTResponseSenderBlock)responseSender
{
  ...
  // return error directly by jacky
  if([request.URL.absoluteString rangeOfString:@"127.0.0.1"].location == NSNotFound){
    [self returnErrorData: task];
    return;
  }
...
- (void)returnErrorData: (RCTNetworkTask *) task {

  [task dispatchCallback:^{

    NSURLResponse *response = [[NSURLResponse alloc] initWithURL:[NSURL URLWithString:@"test"]
                                                         MIMEType:@"text/plain"
                                            expectedContentLength:100
                                                 textEncodingName:@"utf8"];
    task.completionBlock(response, nil, RCTErrorWithMessage(@"http request was disabled."));
